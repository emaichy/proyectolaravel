    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $paciente->Sexo }}</td>
                <td>{{ $paciente->Direccion . ' ' . $paciente->NumeroExterior . ', ' . $paciente->NumeroInterior .' ' . $paciente->CodigoPostal}}</td>
                ...
                <td>
                    ...
                    <form action="{{ route('pacientes.destroy', $paciente) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger btn-sm" onclick="return confirm('¿Está seguro de eliminar este paciente?')">Eliminar</button>
                    </form>
                </td>
            </tr>
            @empty
