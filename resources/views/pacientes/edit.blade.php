        ...
        <form action="{{ route('pacientes.update', $paciente->ID_Paciente) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="Sexo" name="Sexo" required>
                    ...
                    <option value="Masculino" {{ old('Sexo', $paciente->Sexo) == 'Masculino' ? 'selected' : '' }}>Masculino
                    </option>
                    <option value="Femenino" {{ old('Sexo', $paciente->Sexo) == 'Femenino' ? 'selected' : '' }}>Femenino
                    </option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="TipoPaciente" name="TipoPaciente" required>
                    <option value="">Seleccione</option>
                    <option value="Adulto"
                        {{ old('TipoPaciente', $paciente->TipoPaciente) == 'Adulto' ? 'selected' : '' }}>Adulto</option>
                    <option value="Pediátrico"
                        {{ old('TipoPaciente', $paciente->TipoPaciente) == 'Pediátrico' ? 'selected' : '' }}>Pediátrico
                    </option>
                </select>
            </div>
            ...
            <!-- Foto_Paciente -->
            <div class="mb-3">
                <label for="Foto_Paciente" class="form-label">Foto del Paciente</label><br>
                ...
                <input type="file" class="form-control mt-2" id="Foto_Paciente" name="Foto_Paciente">
                <small class="form-text text-muted">Puedes subir una nueva imagen y se eliminará la anterior
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch(`/municipiosEstado/${estadoId}`)
                ...
                .then(data => {
                    if (data.length > 0) {
                        ...
                        municipioSelect.innerHTML = '<option value="">No hay municipios para este estado</option>';
                    }
                })
                .catch(error => {
    ...
    // Cargar municipios al iniciar si ya hay un estado seleccionado (modo edición)
